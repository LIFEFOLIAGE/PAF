	...
	return function url(formio) {
		const xhrRequest = (url, name, query, data, options, progressCallback, abortCallback) => {
			return new NativePromise((resolve, reject) => {
				...
				const token =  (context.getAccessToken) ? context.getAccessToken() : formio.getToken();
				if (token) {
					xhr.setRequestHeader('Authorization', token);
				}
				const csrsToken = context.csrsToken;
				if (csrsToken) {
					xhr.setRequestHeader(csrsToken.headerName, csrsToken.token);
				}
